...
final class TripsViewModel: ObservableObject {
    ...
    init(service: TripsService = TripsService(),
         initialItinerary: [TripLeg] = [],
         initialCurrentLocation: CurrentLocation? = nil) {
    ...
    private func checkAuthStatus() {
        ...
        do {
            if let token = try keychainStore.fetchToken() {
                if token.isEmpty {
                    print("âŒ Token is empty")
                } else {
                    print("âœ… Token exists (length: \(token.count))")
                    
                    if token.components(separatedBy: ".").count == 3 {
                        print("âœ… Token appears to be a valid JWT format")
                    } else {
                        print("âš ï¸ Token doesn't look like a JWT")
                    }
                }
            } else {
                print("âŒ No token found in keychain")
                print("ðŸ’¡ User needs to log in first")
            }
        } catch {
            print("âŒ Error reading token from keychain: \(error)")
        }
    }
    #endif
}
